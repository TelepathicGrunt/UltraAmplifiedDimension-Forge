            ...
            if (currentBlock == defaultBlock) {
                ...
                if (above2Block.getMaterial() == Material.AIR) {
                    ...
                    if (!above2Block.getFluidState().isEmpty())
                        chunkIn.setBlockState(blockpos$Mutable.setPos(x, y, z).move(Direction.UP, 2), middle, false);
                    if (!above3Block.getFluidState().isEmpty())
                        chunkIn.setBlockState(blockpos$Mutable.setPos(x, y, z).move(Direction.UP, 3), middle, false);
                    if (!above4Block.getFluidState().isEmpty())
                        chunkIn.setBlockState(blockpos$Mutable.setPos(x, y, z).move(Direction.UP, 4), middle, false);
                }
