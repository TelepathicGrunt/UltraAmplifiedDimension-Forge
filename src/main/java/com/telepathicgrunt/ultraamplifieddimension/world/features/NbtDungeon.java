...
public class NbtDungeon extends Feature<NbtDungeonConfig>{
    ...
    public boolean generate(ISeedReader world, ChunkGenerator chunkGenerator, Random random, BlockPos position, NbtDungeonConfig config) {
        ...
        if (wallOpenings >= config.minAirSpace) {
            ...
            Optional<StructureProcessorList> processor = world.getWorld().getServer().getDynamicRegistries().getRegistry(Registry.STRUCTURE_PROCESSOR_LIST_KEY).getOptional(config.processor);
            ...
            Optional<StructureProcessorList> postProcessor = world.getWorld().getServer().getDynamicRegistries().getRegistry(Registry.STRUCTURE_PROCESSOR_LIST_KEY).getOptional(config.postProcessor);
            postProcessor.orElse(ProcessorLists.EMPTY).func_242919_a().forEach(placementsettings::addProcessor); // add all post processors
            List<Template.BlockInfo> list = placementsettings.func_237132_a_(((TemplateAccessor)template).uad_getBlocks(), mutable).func_237157_a_();
