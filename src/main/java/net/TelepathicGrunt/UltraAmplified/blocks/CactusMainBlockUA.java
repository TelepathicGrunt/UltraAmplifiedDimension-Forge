...
public class CactusMainBlockUA extends DirectionalBlock implements net.minecraftforge.common.IPlantable {
	...
	public CactusMainBlockUA() {
		...
		this.setDefaultState(
				this.stateContainer.getBaseState().with(AGE, Integer.valueOf(0)).with(FACING, Direction.UP));
	}
	...
	public void tick(BlockState state, World worldIn, BlockPos pos, Random random) {
		...
		if (!state.isValidPosition(worldIn, pos)) {
			...
			if (worldIn.isAirBlock(blockpos)) {
	            ...
	            for(i = 1; 
	            		worldIn.getBlockState(pos.down(i)).getBlock() == this ||
	            		worldIn.getBlockState(pos.down(i)).getBlock() == BlocksInit.CACTUSBODYBLOCKUA ||
	            		worldIn.getBlockState(pos.down(i)).getBlock() == BlocksInit.CACTUSCORNERBLOCKUA; 
	            	++i)
	            { ; }
