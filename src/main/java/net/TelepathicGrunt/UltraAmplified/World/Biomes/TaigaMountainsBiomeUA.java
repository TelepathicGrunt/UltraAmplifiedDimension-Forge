...
public class TaigaMountainsBiomeUA extends BiomeUA {
	   public TaigaMountainsBiomeUA() {
		      super((new Biome.BiomeBuilder()).surfaceBuilder(new CompositeSurfaceBuilder<>(DEFAULT_SURFACE_BUILDER, GRASS_DIRT_GRAVEL_SURFACE)).precipitation(Biome.RainType.RAIN).category(Biome.Category.TAIGA).depth(0.3F).scale(0.4F).temperature(0.44F).downfall(0.8F).waterColor(4159204).waterFogColor(329011).parent("taiga"));
		      ...
		      this.addFeature(GenerationStage.Decoration.UNDERGROUND_ORES, createCompositeFeature(Feature.MINABLE, new MinableConfig(MinableConfig.IS_ROCK, Blocks.IRON_ORE.getDefaultState(), 7), COUNT_RANGE, new CountRangeConfig(Config.ironOreSpawnrate/25, Config.seaLevel-18, 0, 15))); 
		      ...
		      this.addFeature(GenerationStage.Decoration.LOCAL_MODIFICATIONS, createCompositeFeature(FeatureUA.SMALL_BOULDER, new BlockBlobConfig(Blocks.MOSSY_COBBLESTONE, 0), TOP_SOLID, new FrequencyConfig(2)));
		      
		      this.addFeature(GenerationStage.Decoration.VEGETAL_DECORATION, createCompositeFeature(FeatureUA.TAIGA_M_TREE, IFeatureConfig.NO_FEATURE_CONFIG, AT_SURFACE_WITH_EXTRA_UA, new AtSurfaceWithExtraConfig(10, 0.1F, 1)));
		      ...
		      this.addFeature(GenerationStage.Decoration.VEGETAL_DECORATION, createCompositeFeature(Feature.TAIGA_GRASS, IFeatureConfig.NO_FEATURE_CONFIG, RANDOM_POSITION_EVERY_5_HEIGHT, new FrequencyConfig(1)));
