...
public class BiomeMesaUA extends BiomeExtendedUA
{
    ...
    public void genTerrainBlocks(World worldIn, Random rand, ChunkPrimer chunkPrimerIn, int x, int z, double noiseVal)
    {
        ...
        if (this.brycePillars)
        {
            ...
            if (d0 > -2.5D)
            {
                double d2 = Math.abs(this.pillarRoofNoise.getValue((double)i * 1500.001953125D, (double)j * 1500.001953125D));
                d4 = d0 * d0 * 8.5D;
                double d3 = Math.ceil(d2 * 1200.0D) + 1000.0D;
        ...
        for (int j1 = 255; j1 >= 0; --j1)
        {
            ...
            else if (i1 < 15 || this.brycePillars)
            {
                ...
                else if (iblockstate1.getBlock() == Blocks.STONE || iblockstate1.getBlock() == Blocks.END_STONE || iblockstate1.getBlock() == Blocks.NETHERRACK)
                {
                    if (l == -1)
                    {
                        flag1 = false;
