...
public class SnowyTaigaMountainsBiomeUA  extends BiomeUA {
	   public SnowyTaigaMountainsBiomeUA() {
		      super((new Biome.BiomeBuilder()).surfaceBuilder(new CompositeSurfaceBuilder<>(DEFAULT_SURFACE_BUILDER, GRASS_DIRT_GRAVEL_SURFACE)).precipitation(Biome.RainType.SNOW).category(Biome.Category.TAIGA).depth(0.3F).scale(0.4F).temperature(-0.5F).downfall(0.4F).waterColor(11854335).waterFogColor(11847915).parent("snowy_taiga"));
		      if(Config.mineshaftAbovegroundAllowed || Config.mineshaftUndergroundAllowed)
		      this.addStructure(FeatureUA.MINESHAFT_UA, new MineshaftConfigUA((double)Config.mineshaftSpawnrate, MapGenMineshaftUA.Type.COLDORBIRCH));
