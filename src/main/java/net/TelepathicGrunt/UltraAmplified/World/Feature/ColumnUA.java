...
public class ColumnUA extends Feature<ColumnBlocksConfig> 
{
    ...
    public ColumnUA(Function<Dynamic<?>, ? extends ColumnBlocksConfig> configFactoryIn) {
        ...
        for (int y = -2; y <= heightDiff + 2; y++)
        {
	        ...
	        for (int x = position.getX() - widthAtHeight - xMod -1; x <= position.getX() + widthAtHeight + xMod + 1; ++x)
	        {
	            for (int z = position.getZ() - widthAtHeight - zMod - 1; z <= position.getZ() + widthAtHeight + zMod + 1; ++z)
	            {
                    ...
                    boolean flagImperfection3 = this.noiseGen.func_205563_a(x*0.5D, z*0.5D, y*0.5D) < -0;
