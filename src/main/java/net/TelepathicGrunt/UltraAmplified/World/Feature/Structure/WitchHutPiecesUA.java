...
public class WitchHutPiecesUA extends ScatteredStructurePiece {
   ...
   public boolean addComponentParts(IWorld worldIn, Random random, MutableBoundingBox structureBoundingBoxIn, ChunkPos chunkPos) {
      if (!this.isInsideBounds(worldIn, structureBoundingBoxIn, 0)) {
         ...
         this.setBlockState(worldIn, Blocks.CAULDRON.getDefaultState().with(CauldronBlock.LEVEL, 2), 4, 2, 6, structureBoundingBoxIn);
         ...
         this.setBlockState(worldIn, iblockstate.with(StairsBlock.SHAPE, StairsShape.OUTER_RIGHT), 0, 4, 1, structureBoundingBoxIn);
         this.setBlockState(worldIn, iblockstate.with(StairsBlock.SHAPE, StairsShape.OUTER_LEFT), 6, 4, 1, structureBoundingBoxIn);
         this.setBlockState(worldIn, iblockstate3.with(StairsBlock.SHAPE, StairsShape.OUTER_LEFT), 0, 4, 8, structureBoundingBoxIn);
         this.setBlockState(worldIn, iblockstate3.with(StairsBlock.SHAPE, StairsShape.OUTER_RIGHT), 6, 4, 8, structureBoundingBoxIn);
         ...
         if (!this.witch) {
            ...
            if (structureBoundingBoxIn.isVecInside(new BlockPos(l, i1, k))) {
               ...
               for(int i = 0; i < 2; i++) {
                   ...
                   entitywitch.onInitialSpawn(worldIn, worldIn.getDifficultyForLocation(new BlockPos(l, i1, k)), SpawnReason.STRUCTURE, (ILivingEntityData)null, (CompoundNBT)null);
