...
public class EndCityUA extends Structure<EndCityConfig> 
{
    ...
    protected boolean hasStartAt(IChunkGenerator<?> chunkGen, Random rand, int chunkPosX, int chunkPosZ) {
         ...
         if (chunkPosX == chunkpos.x && chunkPosZ == chunkpos.z) {
            ...
            if (!ConfigUA.endCityGeneration || !chunkGen.hasStructure(biome, FeatureUA.END_CITY_UA)) {
               return false;
            } else {
    ...
    protected boolean isEnabledIn(IWorld worldIn) {
     ...
     protected StructureStart makeStart(IWorld worldIn, IChunkGenerator<?> generator, SharedSeedRandom random, int x, int z) {
        ...
        return new EndCityUA.Start(worldIn, generator, random, x, z, biome);
     }
     ...
     private static int getYPosForStructure(int chunkX, int chunkY, IChunkGenerator<?> generatorIn) {
        ...
        //the 114 is highest height that end city can spawn
        int height = Math.min(Math.min(Math.min(k, l), Math.min(i1, j1)), 114);
        return height;
     }
     ...
     public static class Start extends StructureStart {
        ...
        public Start(IWorld worldIn, IChunkGenerator<?> chunkGenerator, SharedSeedRandom sharedSeed, int chunkX, int chunkZ, Biome biomeIn) {
           ...
           int i = EndCityUA.getYPosForStructure(chunkX, chunkZ, chunkGenerator);
           if (i < 60) {
              this.isSizeable = false;
           } else {
              BlockPos blockpos = new BlockPos(chunkX * 16 + 8, i, chunkZ * 16 + 8);
              EndCityPiecesUA.startHouseTower(worldIn.getSaveHandler().getStructureTemplateManager(), blockpos, rotation, this.components, sharedSeed);
