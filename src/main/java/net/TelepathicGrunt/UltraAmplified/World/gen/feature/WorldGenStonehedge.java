...
public class WorldGenStonehedge extends WorldGenerator{
	...
	private static enum stoneHedgeType {
        SIDE,
        CORNER
    }
	...
	public boolean generate(World worldIn, Random rand, BlockPos position) {
		...
		if(position.getY() < 248) 
		{
			...
			System.out.println("StoneHedge | " + position.getX() + " "+position.getZ());
			
			
			//field
			//10% of being a perfect stonehedge right off the bat
			...
			//north stone
			template = pickStonehedgeStyle(stoneHedgeType.SIDE, rand, minecraftserver, templatemanager);
			if(template == null)
			{
				Log.warn("a side stonehedge NTB does not exist!");
				return false;
			}
			...
			//north west stone
			template = pickStonehedgeStyle(stoneHedgeType.CORNER, rand, minecraftserver, templatemanager);;
			if(template == null)
			{
				Log.warn("a corner stonehedge NTB does not exist!");
				return false;
			}
			...
			//center of stonehedge.
			//If all stones are perfect, generates crafting table, otherwise, place a small patch of stones
			if(perfectStoneCount == 8) 
			{
				template =  templatemanager.getTemplate(minecraftserver, new ResourceLocation(UltraAmplified.MOD_ID+":stonehedgeperfectcenter"));
			}
			else 
			{
				template =  templatemanager.getTemplate(minecraftserver, new ResourceLocation(UltraAmplified.MOD_ID+":stonehedgecenter"));
			}
			
			if(template == null)
			{
				Log.warn("a center stonehedge NTB does not exist!");
				return false;
			}
	...
	//picks one out of four templates for the stone hedge
	private Template pickStonehedgeStyle(stoneHedgeType type, Random rand, MinecraftServer minecraftserver, TemplateManager templatemanager) 
	{
		int hedgeType;
		...
		if(type == stoneHedgeType.SIDE) 
		{
			if(hedgeType == 0) 
			{
				return templatemanager.getTemplate(minecraftserver, new ResourceLocation(UltraAmplified.MOD_ID+":stonehedge1"));
			}
			else if(hedgeType == 1) 
			{
				return templatemanager.getTemplate(minecraftserver, new ResourceLocation(UltraAmplified.MOD_ID+":stonehedge2"));
			}
			else if(hedgeType == 2) 
			{
				return templatemanager.getTemplate(minecraftserver, new ResourceLocation(UltraAmplified.MOD_ID+":stonehedge3"));
			}
			else
			{
				return templatemanager.getTemplate(minecraftserver, new ResourceLocation(UltraAmplified.MOD_ID+":stonehedge4"));
			}
		}
		else 
		{
			if(hedgeType == 0) 
			{
				return templatemanager.getTemplate(minecraftserver, new ResourceLocation(UltraAmplified.MOD_ID+":stonehedge5"));
			}
			else if(hedgeType == 1) 
			{
				return templatemanager.getTemplate(minecraftserver, new ResourceLocation(UltraAmplified.MOD_ID+":stonehedge6"));
			}
			else if(hedgeType == 2) 
			{
				return templatemanager.getTemplate(minecraftserver, new ResourceLocation(UltraAmplified.MOD_ID+":stonehedge7"));
			}
			else
			{
				return templatemanager.getTemplate(minecraftserver, new ResourceLocation(UltraAmplified.MOD_ID+":stonehedge8"));
			}
		}
	}
