...
public class BiomeAddModdedFeatures
{
	...
	private static Biome[] vanillaBiomesToCheck = new Biome[] { Biomes.PLAINS, Biomes.DESERT, Biomes.MOUNTAINS, Biomes.FOREST, Biomes.TAIGA, Biomes.SWAMP, Biomes.NETHER, Biomes.THE_END, Biomes.SNOWY_TUNDRA, Biomes.SNOWY_MOUNTAINS, Biomes.MUSHROOM_FIELDS, Biomes.DESERT_HILLS, Biomes.WOODED_HILLS, Biomes.TAIGA_HILLS, Biomes.BAMBOO_JUNGLE, Biomes.BAMBOO_JUNGLE_HILLS, Biomes.JUNGLE, Biomes.JUNGLE_HILLS, Biomes.JUNGLE_EDGE, Biomes.STONE_SHORE, Biomes.SNOWY_BEACH, Biomes.BIRCH_FOREST,
			Biomes.BIRCH_FOREST_HILLS, Biomes.DARK_FOREST, Biomes.SNOWY_TAIGA, Biomes.SNOWY_TAIGA_HILLS, Biomes.GIANT_TREE_TAIGA, Biomes.GIANT_TREE_TAIGA_HILLS, Biomes.WOODED_MOUNTAINS, Biomes.SAVANNA, Biomes.SAVANNA_PLATEAU, Biomes.BADLANDS, Biomes.WOODED_BADLANDS_PLATEAU, Biomes.BADLANDS_PLATEAU, Biomes.SUNFLOWER_PLAINS, Biomes.DESERT_LAKES, Biomes.GRAVELLY_MOUNTAINS, Biomes.FLOWER_FOREST, Biomes.TAIGA_MOUNTAINS, Biomes.SWAMP_HILLS, Biomes.ICE_SPIKES, Biomes.MODIFIED_JUNGLE,
			Biomes.MODIFIED_JUNGLE_EDGE, Biomes.TALL_BIRCH_FOREST, Biomes.TALL_BIRCH_HILLS, Biomes.DARK_FOREST_HILLS, Biomes.SNOWY_TAIGA_MOUNTAINS, Biomes.GIANT_SPRUCE_TAIGA, Biomes.GIANT_SPRUCE_TAIGA_HILLS, Biomes.MODIFIED_GRAVELLY_MOUNTAINS, Biomes.SHATTERED_SAVANNA, Biomes.SHATTERED_SAVANNA_PLATEAU, Biomes.ERODED_BADLANDS, Biomes.MODIFIED_WOODED_BADLANDS_PLATEAU, Biomes.MODIFIED_BADLANDS_PLATEAU, Biomes.FROZEN_OCEAN, Biomes.COLD_OCEAN, Biomes.OCEAN, Biomes.LUKEWARM_OCEAN, Biomes.WARM_OCEAN,
		...
		for (Decoration decorationType : Decoration.values())
		{
			for (ConfiguredFeature<?, ?> feature : vanillaBiome.getFeatures(decorationType))
			{
				...
				else if (UltraAmplified.UAConfig.importModdedFeatures.get())
				{
					...
					// SphereReplaceConfig so we have to dig for it
					...
					// modded blobs are buried in the state variable within the
					// BlockBlobConfig so we have to dig for it
					else if (insideFeature instanceof BlockBlobFeature)
					{
						BlockBlobConfig doublePlantConfig = (BlockBlobConfig) insideConfig.feature.config;
					...
					else if (insideFeature instanceof MultipleWithChanceRandomFeature)
					{
						...
						insideFeature = randConfig.features.get(0).feature.feature;
		...
		for (Biome uaBiome : UABiomes.getRegisteredBiomeArray())
		{
			...
			for (Decoration decorationType : Decoration.values())
			{
				for (int featureIndex = uaBiome.getFeatures(decorationType).size() - 1; featureIndex >= 0; featureIndex--)
				{
					...
					DecoratedFeatureConfig insideConfig = (DecoratedFeatureConfig) uaBiome.getFeatures(decorationType).get(featureIndex).config;
					...
					{
						if (!listOfVanillaStructureFeatures.contains(insideFeature) && !namespace.equals(UltraAmplified.MODID))
						{
					...
					{
						BlockBlobConfig blockBlobConfig = (BlockBlobConfig) insideConfig.feature.config;
						if (checkIfBlockIsAllowed(blockBlobConfig.state.getBlock()))
						{
							uaBiome.getFeatures(decorationType).remove(featureIndex);
						}
					}
