	...
	public BambooJungleBiomeUA()
	{
		...
		this.addFeature(GenerationStage.Decoration.VEGETAL_DECORATION,
				Feature.RANDOM_SELECTOR.configure(new MultipleRandomFeatureConfig(
						ImmutableList.of(Feature.FANCY_TREE.configure(FeatureUA.getConfiguredForUndergroundTreeConfig(DefaultBiomeFeatures.FANCY_TREE_CONFIG)).withChance(0.05F), FeatureUA.JUNGLE_BUSH_UA.configure(FeatureUA.getConfiguredForUndergroundTreeConfig(DefaultBiomeFeatures.JUNGLE_GROUND_BUSH_CONFIG)).withChance(0.15F), Feature.MEGA_JUNGLE_TREE.configure(FeatureUA.getConfiguredForUndergroundTreeConfig(DefaultBiomeFeatures.MEGA_JUNGLE_TREE_CONFIG)).withChance(0.7F)),
						Feature.NORMAL_TREE.configure(FeatureUA.getConfiguredForUndergroundTreeConfig(DefaultBiomeFeatures.JUNGLE_TREE_CONFIG)))).createDecoratedFeature(AT_SURFACE_BELOW_TOP_LAYER_WITH_EXTRA_UA.configure(new AtSurfaceWithExtraConfig(5, 0.1F, 1))));
		this.addFeature(GenerationStage.Decoration.VEGETAL_DECORATION, FeatureUA.BAMBOO_UA.configure(IFeatureConfig.NO_FEATURE_CONFIG).createDecoratedFeature(CHANCE_ON_ALL_SURFACES_UA.configure(new PercentageAndFrequencyConfig(0.85F, 115))));
