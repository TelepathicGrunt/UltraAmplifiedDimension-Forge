...
public class EndFieldBiomeUA extends UABiome
{
	public EndFieldBiomeUA()
	{
		...
		this.addFeature(GenerationStage.Decoration.RAW_GENERATION, Feature.END_ISLAND.configure(IFeatureConfig.NO_FEATURE_CONFIG).createDecoratedFeature(UAPlacement.HEIGHT_BIASED_END_ISLAND_RANGE.configure(new CountRangeConfig(6, 200, 0, 254))));
		this.addFeature(GenerationStage.Decoration.RAW_GENERATION, Feature.END_ISLAND.configure(IFeatureConfig.NO_FEATURE_CONFIG).createDecoratedFeature(UAPlacement.HEIGHT_BIASED_END_ISLAND_RANGE.configure(new CountRangeConfig(3, 140, 0, 199))));
		this.addFeature(GenerationStage.Decoration.RAW_GENERATION, Feature.END_ISLAND.configure(IFeatureConfig.NO_FEATURE_CONFIG).createDecoratedFeature(UAPlacement.HEIGHT_BIASED_END_ISLAND_RANGE.configure(new CountRangeConfig(1, 75, 0, 139))));
		this.addFeature(GenerationStage.Decoration.RAW_GENERATION, Feature.END_ISLAND.configure(IFeatureConfig.NO_FEATURE_CONFIG).createDecoratedFeature(UAPlacement.HEIGHT_BIASED_END_ISLAND_RANGE.configure(new CountRangeConfig(2, 10, 0, 50))));
		...
		this.addFeature(GenerationStage.Decoration.VEGETAL_DECORATION, Feature.CHORUS_PLANT.configure(IFeatureConfig.NO_FEATURE_CONFIG).createDecoratedFeature(UAPlacement.HEIGHT_BIASED_RANGE.configure(new CountRangeConfig(130, 75, 10, 255))));
		this.addFeature(GenerationStage.Decoration.VEGETAL_DECORATION, Feature.CHORUS_PLANT.configure(IFeatureConfig.NO_FEATURE_CONFIG).createDecoratedFeature(UAPlacement.HEIGHT_BIASED_RANGE.configure(new CountRangeConfig(45, 10, 10, 75))));
		this.addFeature(GenerationStage.Decoration.VEGETAL_DECORATION, FeatureUA.END_TREE.configure(FeatureUA.getConfiguredForUndergroundTreeConfig(DefaultBiomeFeatures.PINE_TREE_CONFIG)).createDecoratedFeature(UAPlacement.AT_SURFACE_WITH_EXTRA.configure(new AtSurfaceWithExtraConfig(0, 0.2F, 1))));
		this.addFeature(GenerationStage.Decoration.VEGETAL_DECORATION, FeatureUA.END_TREE.configure(FeatureUA.getConfiguredForUndergroundTreeConfig(DefaultBiomeFeatures.PINE_TREE_CONFIG)).createDecoratedFeature(UAPlacement.AT_SURFACE_BELOW_TOP_LAYER_WITH_EXTRA.configure(new AtSurfaceWithExtraConfig(0, 0.2F, 1))));
		this.addFeature(GenerationStage.Decoration.VEGETAL_DECORATION, Feature.field_227248_z_.configure(FeatureUA.getConfiguredClusterConfig(DefaultBiomeFeatures.TAIGA_GRASS_CONFIG)).createDecoratedFeature(UAPlacement.CHANCE_ON_ALL_SURFACES.configure(new PercentageAndFrequencyConfig(0.7F, 1))));
		this.addFeature(GenerationStage.Decoration.VEGETAL_DECORATION, Feature.field_227248_z_.configure(FeatureUA.getConfiguredClusterConfig(DefaultBiomeFeatures.SWEET_BERRY_BUSH_CONFIG)).createDecoratedFeature(UAPlacement.CHANCE_ON_ALL_SURFACES.configure(new PercentageAndFrequencyConfig(0.05F, 1))));
		this.addFeature(GenerationStage.Decoration.VEGETAL_DECORATION, FeatureUA.SINGLE_BLOCK.configure(new BlockConfig(Blocks.DRAGON_HEAD)).createDecoratedFeature(UAPlacement.HEIGHT_BIASED_RANGE.configure(new CountRangeConfig(2, 10, 1, 50))));
		this.addFeature(GenerationStage.Decoration.VEGETAL_DECORATION, FeatureUA.SINGLE_BLOCK.configure(new BlockConfig(Blocks.SHULKER_BOX)).createDecoratedFeature(UAPlacement.HEIGHT_BIASED_RANGE.configure(new CountRangeConfig(1, 25, 1, 70))));
		this.addFeature(GenerationStage.Decoration.VEGETAL_DECORATION, FeatureUA.SINGLE_BLOCK.configure(new BlockConfig(Blocks.DRAGON_EGG)).createDecoratedFeature(UAPlacement.HEIGHT_BIASED_RANGE.configure(new CountRangeConfig(1, 10, 1, 255))));
