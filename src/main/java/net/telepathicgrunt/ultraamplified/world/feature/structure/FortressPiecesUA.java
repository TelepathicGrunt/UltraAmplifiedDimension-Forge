...
public class FortressPiecesUA
{
	...
	public static class Corridor extends FortressPiecesUA.Piece
	{
		...
		public boolean create(IWorld world, ChunkGenerator<?> p_225577_2_, Random random, MutableBoundingBox structureBoundingBoxIn, ChunkPos p_74875_4_)
		{
			...
			if (UltraAmplified.UAStructuresConfig.chestGeneration.get() && (this.stoneVariant ? random.nextInt(9) == 0 : random.nextInt(6) == 0) && structureBoundingBoxIn.isVecInside(new BlockPos(this.getXWithOffset(3, 3), this.getYWithOffset(2), this.getZWithOffset(3, 3))))
			{
	...
	public static class Corridor2 extends FortressPiecesUA.Piece
	{
		...
		public boolean create(IWorld world, ChunkGenerator<?> p_225577_2_, Random random, MutableBoundingBox structureBoundingBoxIn, ChunkPos p_74875_4_)
		{
			...
			if (UltraAmplified.UAStructuresConfig.chestGeneration.get() && (this.stoneVariant ? random.nextInt(9) == 0 : random.nextInt(6) == 0) && structureBoundingBoxIn.isVecInside(new BlockPos(this.getXWithOffset(1, 3), this.getYWithOffset(2), this.getZWithOffset(1, 3))))
			{
		...
		protected BlockState getStoneVariantBlockState(BlockState blockstateIn, Random rand)
		{
			...
			if (this.stoneVariant)
			{
				if (block == Blocks.NETHER_BRICKS)
				{
					...
					float silverfishThreshold = (float) (UltraAmplified.UAStructuresConfig.silverfishStrongholdSpawnrate.get() / 100);
		...
		protected void fillWithRandomBlocks(IWorld world, MutableBoundingBox boundingboxIn, int xMin, int yMin, int zMin, int xMax, int yMax, int zMax, BlockState boundaryBlockState, BlockState insideBlockState, boolean existingOnly, Random rand)
		{
			for (int i = yMin; i <= yMax; ++i)
			{
				for (int j = xMin; j <= xMax; ++j)
				{
					...
					{
						if (!existingOnly || this.getBlockStateFromPos(world, j, i, k, boundingboxIn).getMaterial() != Material.AIR)
						{
							...
							{
								this.setBlockState(world, getStoneVariantBlockState(boundaryBlockState.getBlock().getDefaultState(), rand), j, i, k, boundingboxIn);
							}
						}
					}
				}
			...
			if (!this.hasSpawner)
			{
				...
				if (structureBoundingBoxIn.isVecInside(blockpos))
				{
					...
					if (stoneVariant)
					{
						...
						if (UltraAmplified.UAStructuresConfig.allowSilverfishSpawnerFortress.get())
						{
