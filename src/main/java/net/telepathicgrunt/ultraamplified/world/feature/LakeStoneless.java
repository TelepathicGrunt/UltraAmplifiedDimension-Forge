...
public class LakeStoneless extends Feature<BlockStateFeatureConfig>
{
    ...
    {
	if (configBlock.state.getBlock() == Blocks.SLIME_BLOCK) {
	    int modifier = UltraAmplified.UAFeaturesConfig.slimeLakeGen.get();
	    if (modifier == 1000000 || (int) (random.nextFloat() * modifier) != 0) {
		return false;
	    }
	}
	else if (configBlock.state.getBlock() == Blocks.HONEY_BLOCK) {
	    int modifier = UltraAmplified.UAFeaturesConfig.honeyLakeGen.get();
	    if (modifier == 1000000 || (int) (random.nextFloat() * modifier) != 0) {
		return false;
	    }
	}
