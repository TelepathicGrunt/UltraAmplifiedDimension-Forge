...
public class LakeStoned extends Feature<BlockStateFeatureConfig>
{
	...
	if (configBlock.state.getBlock() == Blocks.WATER) {
	    int modifier = UltraAmplified.UAFeaturesConfig.waterLakeGen.get();
	    if (modifier == 1000000 || (int) (random.nextFloat() * modifier) != 0) {
		return false;
	    }
	}
	else if (configBlock.state.getBlock() == Blocks.LAVA) {
	    int modifier = UltraAmplified.UAFeaturesConfig.lavaLakeGen.get();
	    if (modifier == 1000000 || (int) (random.nextFloat() * modifier) != 0) {
		return false;
	    }
	}
