...
public class ColumnRamp extends Feature<ColumnBlocksConfig>
{
	...
	public ColumnRamp(Function<Dynamic<?>, ? extends ColumnBlocksConfig> configFactory)
	{
		...
		irreplacableBlocks = ImmutableSet.of(Blocks.BEEHIVE, Blocks.AIR, Blocks.CAVE_AIR, Blocks.BROWN_MUSHROOM_BLOCK, Blocks.RED_MUSHROOM_BLOCK, Blocks.MUSHROOM_STEM, Blocks.CACTUS, UABlocks.CACTUSBODYBLOCKUA.get(), UABlocks.CACTUSCORNERBLOCKUA.get(), UABlocks.CACTUSMAINBLOCKUA.get());
	}
	...
	public boolean place(IWorld world, ChunkGenerator<? extends GenerationSettings> changedBlock, Random rand, BlockPos position, ColumnBlocksConfig blocksConfig)
	{
		...
		if (!UltraAmplified.UAFeaturesConfig.rampGen.get())
		{
			return false;
		}
			...
			for (int x = position.getX() - widthAtHeight - 1; x <= position.getX() + widthAtHeight + 1; ++x)
			{
				for (int z = position.getZ() - widthAtHeight - 1; z <= position.getZ() + widthAtHeight + 1; ++z)
				{
					...
					if (!block.isIn(BlockTags.LEAVES) && !block.isIn(BlockTags.LOGS) && !irreplacableBlocks.contains(block.getBlock()) && xzDiffSquaredStretched <= circleBounds)
					{
						...
						world.setBlockState(blockpos$Mutable, UltraAmplified.UATerrainConfig.lavaOcean.get() ? LAVA : WATER, 2);
					    else 
