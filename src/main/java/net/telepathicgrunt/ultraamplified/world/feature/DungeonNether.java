        ...
        for (int x = xMin; x <= xMax; ++x)
        {
            for (int y = -1; y <= 4; ++y)
            {
                for (int z = zMin; z <= zMax; ++z)
                {
                    ...
                    {
                    	if((world.isAirBlock(blockpos$Mutable) || world.getBlockState(blockpos$Mutable).getMaterial() == Material.LAVA)
                    		&& (world.isAirBlock(blockpos$Mutable.up()) || world.getBlockState(blockpos$Mutable.up()).getMaterial() == Material.LAVA))
                    	{
                            ++validOpenings;
                    	}
                    }
