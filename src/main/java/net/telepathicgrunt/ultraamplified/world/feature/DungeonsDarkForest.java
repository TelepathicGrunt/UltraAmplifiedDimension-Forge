...
public class DungeonsDarkForest extends Feature<NoFeatureConfig> 
{
    public DungeonsDarkForest(Function<Dynamic<?>, ? extends NoFeatureConfig> configFactoryIn) {
                ...
                for (int i4 = 3; i4 >= -1; --i4)
                {
                    for (int k4 = l1; k4 <= i2; ++k4)
                    {
                        ...
                        else if (worldIn.getBlockState(blockpos1).getMaterial().isSolid() && worldIn.getBlockState(blockpos1).getBlock() != Blocks.CHEST && worldIn.getBlockState(blockpos1).getBlock() != Blocks.SPAWNER)
                        {
                            ...
                            {
                                worldIn.setBlockState(blockpos1, Blocks.DARK_OAK_PLANKS.getDefaultState(), 2);
                            }
                            else
                            {
                            	if(rand.nextInt(3) == 0) 
                            	{
                                    worldIn.setBlockState(blockpos1, Blocks.DARK_OAK_LOG.getDefaultState(), 2);
                            	}
                            	else {
                            		worldIn.setBlockState(blockpos1, Blocks.DARK_OAK_LEAVES.getDefaultState(), 2);
                            	}
                            }
                        }
                    }
