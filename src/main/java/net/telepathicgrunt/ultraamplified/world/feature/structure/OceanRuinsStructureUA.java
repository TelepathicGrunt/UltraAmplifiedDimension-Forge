...
public class OceanRuinsStructureUA extends Structure<OceanRuinConfig>
{
	...
	{
		int maxDistance = UltraAmplified.UAConfig.oceanRuinsSpawnrate.get();
		int minDistance = 8;
		if (maxDistance < 9)
		{
	...
	public boolean shouldStartAt(BiomeManager p_225558_1_, ChunkGenerator<?> chunkGen, Random rand, int chunkPosX, int chunkPosZ, Biome biome)
	{
		...
		{
			if (UltraAmplified.UAConfig.oceanRuinsSpawnrate.get() != 101 && chunkGen.hasStructure(biome, this))
			{
				return true;
			}
		}
