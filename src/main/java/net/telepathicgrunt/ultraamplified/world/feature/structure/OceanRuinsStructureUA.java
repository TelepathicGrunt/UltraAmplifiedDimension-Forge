...
public class OceanRuinsStructureUA extends Structure<OceanRuinConfig>
{
	...
	{
		int maxDistance = UltraAmplified.UAStructuresConfig.oceanRuinsSpawnrate.get();
		int minDistance = 8;
		if (maxDistance < 9)
		{
	...
	public boolean canBeGenerated(BiomeManager p_225558_1_, ChunkGenerator<?> chunkGen, Random rand, int chunkPosX, int chunkPosZ, Biome biome)
	{
		...
		{
			if (UltraAmplified.UAStructuresConfig.oceanRuinsSpawnrate.get() != 101 && chunkGen.hasStructure(biome, this))
			{
				return true;
			}
		}
