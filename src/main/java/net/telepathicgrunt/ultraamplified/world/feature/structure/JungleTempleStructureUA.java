...
public class JungleTempleStructureUA extends Structure<NoFeatureConfig>
{
	...
	{
		int maxDistance = UltraAmplified.UAStructuresConfig.jungleTempleSpawnrate.get();
		int minDistance = 8;
		if (maxDistance < 9)
		{
	...
	public boolean canBeGenerated(BiomeManager p_225558_1_, ChunkGenerator<?> chunkGen, Random rand, int chunkPosX, int chunkPosZ, Biome biome)
	{
		...
		{
			if (UltraAmplified.UAStructuresConfig.jungleTempleSpawnrate.get() != 101 && chunkGen.hasStructure(biome, this))
			{
				return true;
			}
		}
