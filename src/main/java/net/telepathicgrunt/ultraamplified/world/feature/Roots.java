    ...
    public boolean place(IWorld worldIn, ChunkGenerator<? extends GenerationSettings> changedBlock, Random rand, BlockPos position, BlockConfig blockConfig)
    {
		...
		for(int rootNum = 1; rootNum < numOfRoots+1; rootNum++){
			...
			for(int length = 0; length < rootLength; length++){
				...
				xOffset = (int)MathHelper.clamp(this.noiseGen.func_205563_a(Mutable.getX() * 2D+20000*rootNum, Mutable.getZ() * 2D+20000*rootNum, Mutable.getY()*1D+20000*rootNum) * 50.0D, -1, 1);
				zOffset = (int)MathHelper.clamp(this.noiseGen.func_205563_a(Mutable.getX() * 2D+10000*rootNum, Mutable.getZ() * 2D+10000*rootNum, Mutable.getY()*1D+10000*rootNum) * 50.0D, -1, 1);
				yOffset = (int)MathHelper.clamp(this.noiseGen.func_205563_a(Mutable.getX() * 2D-10000*rootNum, Mutable.getZ() * 2D-10000*rootNum, Mutable.getY()*1D-10000) * 50.0D *rootNum - 1, -1, 1);
